...
//  RSSClient
//
//  Created by Dzmitry Fralou on 27.07.17.
...
class ReachabilityC {
    ...
    static func networkReachabilityForInternetConnection() -> ReachabilityC? {
        var zeroAddress = sockaddr_in()
        zeroAddress.sin_len = UInt8(MemoryLayout.size(ofValue: zeroAddress))
        zeroAddress.sin_family = sa_family_t(AF_INET)
        return ReachabilityC(hostAddress: zeroAddress)
    }
    
    static func networkReachabilityForLocalWiFi() -> ReachabilityC? {
        ...
        return ReachabilityC(hostAddress: localWifiAddress)
    }
    ...
    func startNotifier() -> Bool {
        ...
        guard let reachability = networkReachability, SCNetworkReachabilitySetCallback(reachability, { (target: SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) in
            if let currentInfo = info {
                ...
                if infoObject is ReachabilityC {
                    let networkReachability = infoObject as! ReachabilityC
